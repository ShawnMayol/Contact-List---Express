...
Contact.getAll = (result) => {
    dbConn.query('SELECT * FROM contact_list', (err, res) => {
        if (err) {
            console.log('Error:', err);
            result(err, null);
        } else {
...
Contact.getById = (id, result) => {
    dbConn.query('SELECT * FROM contact_list WHERE id = ?', [id], (err, res) => {
        if (err) {
            console.log('Error:', err);
            result(err, null);
...
Contact.update = (id, contact, result) => {
    dbConn.query(
        ...
        [contact.lastName, contact.firstName, contact.emailAddress, contact.contactNumber, id],
        (err, res) => {
            if (err) {
                console.log('Error:', err);
                result(err, null);
